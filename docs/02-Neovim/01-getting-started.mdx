...
There is also a faster way to search a word if it is already under your cursor. Just use the key `*`. Then automatically it will execute a search with `/\<word\>` and jump you to the next occurrence.

:::tip[Trick]

Do not manually type out `/\<word\>` just press `*` on the word!
...
So far I am figuring out my indentation by using `V` to select lines and `=` on the selection to auto-indent (`=` does a [cindent](https://neovim.io/doc/user/options.html#'cindent')). You can even auto-indent an entire paragraph with `=ap` (see [text objects](03-advanced-motions.mdx#text-objects)).

```js
// You can do =ap on this entire section to fix the indents
    const item = {
    ...data
            }
```

To tab, select lines with `V` and then `>` or `<` to indent or unindent. Note: for unknown reasons you may have to press `>` or `<` twice sometimes.
