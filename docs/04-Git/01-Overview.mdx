...
# Change branch name to new_name
...
:::tip[Trick]
Use `git pull --rebase` to avoid merge commits and keep a linear history when pulling updates. Note that this will rebase your local branch **ONTO** the remote branch it is tracking, so downstream branches may be orphaned if you are not careful (see `--update-refs` in [rebasing notes](./02-Advanced.mdx#rebasing-stacked-branches)).
:::
...
One interesting detail I recently learned about `git restore .` is that it resets the working tree to the index state. So the result is that any unstaged changes on tracked files are removed, but staged changes remain. It's easy to think of restoring the working directory as wiping all changes in the tracked files, but what's actually happening is that git takes the version of the file from the index and writes it over the file, thus it is only **unstaged changes** that are wiped.
...
# For partial file changes - stage hunks interactively
git add --patch
git add -p         # short hand
