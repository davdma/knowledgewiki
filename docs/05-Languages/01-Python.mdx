...
There are different ways to parallelize: `multiprocessing`, `concurrent.futures`. You have `ThreadPoolExecutor` and `ProcessPoolExecutor`.

Important distinction is whether the task is I/O bound or CPU bound. If it is I/O bound the recommendation is to use `ThreadPoolExecutor` and threads that share memory.
